...
- (void)openPasscodeWithMode:(int)mode viewController:(UIViewController *)viewController {
    ...
    if (_mode == 0) {
        ...
        [_passcodeViewController setInstructions:@"Enter to unlock"];
    }
}
...
- (void)enteredCode:(NSString *)code {
    if (_mode == 0) {
        if (_count == 0) {
            _prevCode = code;
            [_passcodeViewController setInstructions:@"repeat"];
            ...
            if ([code isEqualToString:_prevCode]) {
                ...
                [_passcodeViewController setInstructions:@"Enter New Code"];
                [_passcodeViewController setErrorMessage:@"Passcode Doesn't match"];
                [_passcodeViewController reset];
                _count = 0;
                return;
            }
        ...
        if ([code isEqualToString:[[DMKeychain defaultKeychain] objectForKey:KEYCHAIN_NAME]]) {
            ...
            if (_count == 1) {
                ...
                [_passcodeViewController setErrorMessage:[NSString stringWithFormat:@"2 attempt left"]];
            }
            [_passcodeViewController reset];
